# ---------------------------------------------------------
# RNA-seq Workflow: P29534 Analysis (edgeR GLM version)
# Mirrors test_2.R workflow but uses edgeR GLM QL (no voom)
# ---------------------------------------------------------
...
# 3. Design Selection
# -----------------------------
choose_full_rank_design <- function(ann) {
  ...
  for (f in candidates) {
    ...
    if (!is.null(mm) && qr(mm)$rank == ncol(mm)) {
      return(list(formula = f, design = mm))
    }
  }
...
# -----------------------------
# 4. edgeR Modeling (GLM QL)
# -----------------------------
run_de_analysis <- function(counts, ann, cfg) {
  message("Running edgeR GLM QL analysis...")

  dge <- DGEList(counts = counts)

  keep <- rowSums(cpm(dge) >= 1) >= ceiling(ncol(dge) * cfg$min_prop)
  dge <- dge[keep, , keep.lib.sizes = FALSE]

  dge <- calcNormFactors(dge, method = "TMM")

  design_info <- choose_full_rank_design(ann)
  design <- design_info$design
  colnames(design) <- make.names(colnames(design))
  message("Using design: ", paste(deparse(design_info$formula), collapse = " "))
  message("Design coefficients: ", paste(colnames(design), collapse = ", "))

  dge <- estimateDisp(dge, design, robust = TRUE)
  fit <- glmQLFit(dge, design, robust = TRUE)

  coef_names <- colnames(design)
  main_coef <- coef_names[grepl("^SensoryStatusSensoryLoss$", coef_names)][1]
  if (is.na(main_coef)) {
    main_coef <- coef_names[grepl("^SensoryStatus", coef_names) & !grepl("timepoint", coef_names)][1]
  }
  int_t1 <- coef_names[grepl("SensoryStatus.*timepoint3_weeks", coef_names)][1]
  int_t2 <- coef_names[grepl("SensoryStatus.*timepoint2_months", coef_names)][1]

  if (is.na(main_coef)) {
    stop("Could not locate the SensoryStatus main-effect coefficient in the edgeR design.")
  }

  make_contrast <- function(main_name, int_name = NA_character_) {
    v <- rep(0, ncol(design))
  ...
  c_t0 <- make_contrast(main_coef)
  c_t1 <- if (!is.na(int_t1)) {
    make_contrast(main_coef, int_t1)
  } else {
    warning("No SensoryStatus:timepoint(3_weeks) interaction coefficient found; using SensoryStatus main effect for T1.")
    make_contrast(main_coef)
  }
  c_t2 <- if (!is.na(int_t2)) {
    make_contrast(main_coef, int_t2)
  } else {
  ...
  results <- list(
    SensoryLoss_vs_Normal_T0 = get_res(c_t0),
    SensoryLoss_vs_Normal_T1 = get_res(c_t1),
    SensoryLoss_vs_Normal_T2 = get_res(c_t2)
  )

  logcpm_mat <- cpm(dge, log = TRUE, prior.count = 2)

  return(list(results = results, expr_mat = logcpm_mat))
}
...
main <- function() {
  ...
  expr_mat <- de_out$expr_mat

  original_ids <- rownames(expr_mat)
  mapped_names <- map_gene_symbols(original_ids, config)
  rownames(expr_mat) <- mapped_names

  id_map <- setNames(mapped_names, original_ids)
  for (nm in names(results)) {
    new_names <- id_map[rownames(results[[nm]])]
    new_names[is.na(new_names)] <- rownames(results[[nm]])[is.na(new_names)]
    rownames(results[[nm]]) <- new_names
  }
  enrichment <- run_go_and_gsea(results, method_name = "edgeR", cfg = config)
