# ---------------------------------------------------------
# Improved RNA-seq Workflow: P29534 Analysis
# Based on edgeR/limma-voom with best practices from req.txt
# ---------------------------------------------------------
...
# -----------------------------
process_differential_expression <- function(counts, ann, cfg) {
  message("Filtering and Normalizing (TMM + Voom)...")
  
  dge <- DGEList(counts = counts)
  
  # Filter low-count genes (per req.txt recommendation)
  keep <- rowSums(cpm(dge) >= 1) >= (ncol(dge) * cfg$min_prop)
  dge <- dge[keep, , keep.lib.sizes = FALSE]
  
  # TMM Normalization (per req.txt recommendation)
  ...
  message("Applying batch correction (ComBat)...")
  batch <- ann$lane
  
  # Standardize covariates
  covs <- intersect(c("sex", "bmi"), colnames(ann))
  prefix <- if (length(covs) > 0) paste(covs, collapse = " + ") else NULL
  
  mod_formula <- if (!is.null(prefix)) {
    as.formula(paste("~", prefix, "+ SensoryStatus + timepoint"))
  } else {
    ~ SensoryStatus + timepoint
  }
  
  mod <- model.matrix(mod_formula, data = ann)
  v$E <- ComBat(dat = v$E, batch = batch, mod = mod)
  
  return(v)
}

# -----------------------------
# 4. Gene Annotation Mapping
# -----------------------------
...
# -----------------------------
run_de_analysis <- function(v, ann) {
  message("Running DE analysis with duplicateCorrelation...")
  
  # Standardize covariates
  covs <- intersect(c("sex", "bmi"), colnames(ann))
  prefix <- if (length(covs) > 0) paste(covs, collapse = " + ") else NULL
  
  design_formula <- if (!is.null(prefix)) {
    as.formula(paste("~", prefix, "+ SensoryStatus * timepoint"))
  } else {
    ~ SensoryStatus * timepoint
  }
  
  design <- model.matrix(design_formula, data = ann)
  colnames(design) <- make.names(colnames(design))
  
  corfit <- duplicateCorrelation(v, design, block = ann$patientID)
  fit <- lmFit(v, design, block = ann$patientID, correlation = corfit$consensus)
  fit <- eBayes(fit)
  
  ct <- makeContrasts(
    SensoryLoss_vs_Normal_T0 = SensoryStatusSensoryLoss,
    SensoryLoss_vs_Normal_T1 = SensoryStatusSensoryLoss + SensoryStatusSensoryLoss.timepoint3_weeks,
    SensoryLoss_vs_Normal_T2 = SensoryStatusSensoryLoss + SensoryStatusSensoryLoss.timepoint2_months,
    levels = design
  )
  ...
  results <- lapply(colnames(ct), function(name) {
    df <- topTable(fit2, coef = name, number = Inf)
    df$logP <- -log10(df$P.Value)
    return(df)
  })
  names(results) <- colnames(ct)
...
run_go_and_gsea <- function(results, method_name, cfg) {
  ...
  run_go <- function(genes, universe, ontology = "BP") {
...
plot_volcano_enhanced <- function(df, title) {
  # Defensive cleaning: remove non-finite values that can break layers
  df <- df[is.finite(df$logFC) & is.finite(df$logP), , drop = FALSE]

  # If nothing to plot, return an empty-but-valid plot
  if (nrow(df) == 0) {
    return(
      ggplot() +
...
plot_heatmap_enhanced <- function(df, v_data, ann_subset, title) {
  # If no samples for this timepoint, skip cleanly
  ...
  # Also ensure the samples exist in v_data
  ...
  ann_col <- ann_subset[match(keep_samples, ann_subset$sample),
                        c("SensoryStatus", "timepoint"), drop = FALSE]
  rownames(ann_col) <- colnames(mat)

  # Final check: ensure annotation has rows before plotting
  if (is.null(ann_col) || nrow(ann_col) == 0 || nrow(mat) == 0 || ncol(mat) == 0) {
    message("Skipping heatmap (insufficient data after filtering): ", title)
    return(invisible(NULL))
  }

  # Remove columns that have 0 non-NA values (to avoid pheatmap error)
...
main <- function() {
  ...
  v <- process_differential_expression(data$counts, data$ann, config)

  # Map names
  mapped_names <- map_gene_symbols(rownames(v$E), config)
  rownames(v$E) <- mapped_names

  # Run DE
  results <- run_de_analysis(v, data$ann)
  #print(results)
  enrichment <- run_go_and_gsea(results, method_name = "voom", cfg = config)
  ...
  for (tp in c("T0", "T1", "T2")) {
    ...
    plot_heatmap_enhanced(results[[res_name]], v$E, ann_sub, paste("Heatmap:", res_name))
  }
